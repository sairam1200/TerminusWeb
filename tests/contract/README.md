...
Real mode fails closed unless both the adapter and candidate SHA are provided, and the SHA must resolve to a commit object in the local repository. A real adapter must export `metadata` and `invoke(testCase)`. The metadata must declare `evidenceClass: 'real-consumer'` and the same `candidateSha` supplied in the environment. The adapter is responsible for loading Session 01's canonical fixtures from the exact checked-out consumer candidate; this harness never copies or guesses those fixtures.
